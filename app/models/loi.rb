class Loi < ApplicationRecord
  ...
  validates_date :p1sigdate, before: lambda {2.days.from_now}, after: lambda {1.years.ago}, before_message: "date is too recent", after_message: "date is too old"
  ...
  def p2sigdate_formatted
    unless p2sigdate.nil?
      p2sigdate.strftime("%m/%d/%Y")
    end
  end
